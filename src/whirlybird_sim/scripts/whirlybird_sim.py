...
class WhirlybirdSim():
    ...
    def dynamics(self, state, command):
        ...
        M31 = -Jx*stheta
        M32 = (Jy-Jz)*sphi*cphi*ctheta
        M33 = (m1*l1**2+m2*l2**2+Jy*sphi**2+Jz*cphi**2)*ctheta**2+Jx*stheta**2
        
        M = np.matrix([[Jx, 0, M13],
                       [0, M22, M23],
                       [M31, M32, M33]])


        c1 = -thetad**2*(Jz-Jy)*sphi*cphi+psid**2*(Jz-Jy)*sphi*cphi*ctheta**2 \
             -thetad*psid*ctheta*(Jx-(Jz-Jy)*(cphi**2-sphi**2))
        
        c2 = psid**2*stheta*ctheta*(-Jx+m1*l1**2+m2*l2**2+Jy*sphi**2+Jz*cphi**2) \
             -2*phid*thetad*(Jz-Jy)*sphi*cphi-phid*psid*ctheta*\
             (-Jx+(Jz-Jy)*(cphi**2-sphi**2))
        
        c3 = thetad**2*(Jz-Jy)*sphi*cphi*stheta-phid*thetad*ctheta* \
             (Jx+(Jz-Jy)*(cphi**2-sphi**2))-2*phid*psid*(Jz-Jy)*ctheta**2* \
             sphi*cphi+2*thetad*psid*stheta*ctheta*(Jx-m1*l1**2-m2*l2**2- \
                                                    Jy*sphi**2-Jz*cphi**2)
        ...
        Q = Q.reshape(3,1)
        M = M.reshape(3,3)
        xdot[3:6] = np.linalg.solve(M,Q-dPdq-c)       
