...
class Controller():
	...
	#Klon = np.matrix([[7.36624816, 1.9098619]]) # these aren't right?
	Klon = np.matrix([[2.14328874, 1.9098619]]) 
	krlon = 2.14328874  
 

	F =  krlon*self.theta_r - Klon*x
	print F,
	self.Fe = 0.85*(m1*l1-m2*l2)*g*cos(theta)/l1
	F += self.Fe
	...
	Klat = np.matrix([[0.34220482, 0.10491429, 0.05628416, 0.14023202]])  # 0.3 tr
	#Klat = np.matrix([[0.08555121, 0.00655714, 0.02814208, 0.017529]])    # 0.6 tr
	#Klat = np.matrix([[0.15209103, 0.02072381, 0.03752278, 0.04155023]])   # 0.45 tr


	krlat = 0.10491429 # 0.3 tr
			   # 0.5 tr
	#krlat = 0.02072381 # 0.45 tr
	...
	print self.tau
        ...
        # Scale Output
        l_out = 0.75*(F+self.tau/d)/2.0/km  ## convert to PWM
        if(l_out < 0):
            l_out = 0
        elif(l_out > 0.7):
            l_out = 0.7
